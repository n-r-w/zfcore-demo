...
// Лимит произведения числа колонок на число строк для отключения автоподгона высоты строк
#define AUTO_RESIZE_LIMIT 5000
...
TableViewBase::TableViewBase(const DataStructurePtr& structure, const DataProperty& dataset_property, QWidget* parent)
    : QTableView(parent)
...
TableViewBase::TableViewBase(const View* view, const DataProperty& dataset_property, QWidget* parent)
    : QTableView(parent)
    , _view(view)
    , _dataset_property(dataset_property)
{
...
TableViewBase::TableViewBase(const DataWidgets* widgets, const DataProperty& dataset_property, const HighlightProcessor* highlight,
                             QWidget* parent)
    : QTableView(parent)
    , _widgets(widgets)
...
DatasetConfigOptions TableViewBase::configMenuOptions() const
{
    return horizontalHeader()->configMenuOptions();
}

void TableViewBase::setConfigMenuOptions(const DatasetConfigOptions& o)
{
...
View* TableViewBase::view() const
{
    return const_cast<View*>(_view);
}

DataWidgets* TableViewBase::widgets() const
{
    return const_cast<DataWidgets*>(_widgets);
}

HighlightProcessor* TableViewBase::highlight() const
{
    return const_cast<HighlightProcessor*>(_highlight);
}

DataProperty TableViewBase::datasetProperty() const
{
    return _dataset_property;
}

DataStructurePtr TableViewBase::structure() const
{
    return _structure;
}
...
QSize TableViewBase::minimumSizeHint() const
{
    return isAutoShrink() ? sizeHint() : QTableView::minimumSizeHint();
}
...
void TableViewBase::keyPressEvent(QKeyEvent* e)
{
    if (e->key() == Qt::Key_Space && e->modifiers() == Qt::NoModifier && currentIndex().isValid()) {
        if (isBooleanColumn(currentIndex().column())) {
            ...
            if (_dataset_property.columns().count() > currentIndex().column()) {
                auto property = _dataset_property.columns().at(currentIndex().column());
                if (property.options().testFlag(PropertyOption::ReadOnly)) {
                    e->ignore();
                    return;
                }
...
bool TableViewBase::edit(const QModelIndex& index, EditTrigger trigger, QEvent* event)
{
...
void TableViewBase::sl_configMenuRequested(const QPoint& pos, const DatasetConfigOptions& options)
{
    int frozen_group = frozenGroupCount();
    Utils::headerConfig(frozen_group, horizontalHeader(), pos, options, true, view(), datasetProperty());
    if (frozen_group >= 0)
        setFrozenGroupCount(frozen_group);
}
...
void TableViewBase::init()
{
    ...
    setHorizontalHeader(new HeaderView(view(), datasetProperty(), Qt::Horizontal, this));
    if (isSortingEnabled())
    ...
    connect(horizontalHeader(), &HeaderView::sg_configMenuRequested, this, &TableViewBase::sl_configMenuRequested);
    ...
    setVerticalHeader(new HeaderView(view(), datasetProperty(), Qt::Vertical, this));
...
{
    return quint64(row_count) * quint64(column_count) < AUTO_RESIZE_LIMIT;
}
