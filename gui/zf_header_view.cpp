...
namespace zf
{
HeaderView::HeaderView(const View* view, const DataProperty& dataset_property, Qt::Orientation orientation, QWidget* parent)
    : QHeaderView(orientation, parent)
    , _view(const_cast<View*>(view))
    , _dataset_property(dataset_property)
{
    if (_view != nullptr) {
        Z_CHECK(dataset_property.isValid() && _view->structure()->contains(dataset_property));

        connect(_view, &View::sg_autoSearchStatusChanged, this, &HeaderView::sl_autoSearchStatusChanged);
        connect(_view, &View::sg_autoSearchTextChanged, this, &HeaderView::sl_autoSearchTextChanged);
    }
    ...
    if (orientation == Qt::Horizontal) {
        ...
        setMinimumSectionSize(UiSizes::defaultTableRowHeight() + Utils::scaleUI(4));
        setSortIndicatorShown(true);
        setSortIndicator(-1, Qt::AscendingOrder);
    } else {
        setVisible(false);

        int default_row_height = UiSizes::defaultTableRowHeight();
    ...
    connect(this, &QHeaderView::customContextMenuRequested, this, [&](const QPoint& pos) {
        if (_config_options != DatasetConfigOptions())
            emit sg_configMenuRequested(pos, _config_options);
    });
...
void HeaderView::paintEvent(QPaintEvent* e)
{
    ...
    if (orientation() == Qt::Vertical) {
        ...
        painter.save();
        painter.setPen(Colors::uiLineColor(true));
    ...
    if (orientation() == Qt::Horizontal && _view != nullptr) {
        if (!_view->conditionFilter(_dataset_property)->isEmpty()) {
            // отображение фильтрации
            painter.save();

            QIcon icon = Utils::resizeSvg(":/share_icons/filter-highlight.svg", Utils::scaleUI(10), true);
            icon.paint(&painter, QRect(Utils::scaleUI(2), Utils::scaleUI(2), Utils::scaleUI(10), Utils::scaleUI(10)));

            painter.restore();
        }

        if (_show_autosearch && _view->isAutoSearchActive() && !_view->autoSearchText().isEmpty()) {
            // отображение автопоиска
            int border = 1;
            int text_border_h = 3;
            int text_border_v = 1;
            QRect all_rect = viewport()->rect();
            if (!stretchLastSection()) {
                int last_visible = _model->rootItem()->lastVisibleSection();
                if (last_visible >= 0)
                    all_rect.setRight(
                        qMin(all_rect.right(), sectionViewportPosition(last_visible) + sectionSize(last_visible)));
            }
            QRect min_rect = all_rect.adjusted(border, border, -border, -border);

            QRect text_rect = painter.boundingRect(min_rect, Qt::AlignCenter, _view->autoSearchText());
            text_rect.moveTopLeft({0, 0});
            QRect rect = text_rect;
            rect.setWidth(qMin(min_rect.width(), rect.width() + text_border_h * 2));
            rect.setHeight(qMin(min_rect.height(), rect.height() + text_border_v * 2));
            rect.moveTop(qMax(0, all_rect.height() - rect.height() - border));
            rect.moveLeft(qMax(0, all_rect.width() - rect.width() - border));
            ...
            painter.save();
            painter.drawText(rect, text_rect.width() > rect.width() ? Qt::AlignRight | Qt::AlignVCenter : Qt::AlignCenter,
                _view->autoSearchText());
            painter.restore();
        }
    }
}
...
QSize HeaderView::sectionSizeFromContents(int logicalIndex) const
{
    ...
    /* Вот так на самом деле внутри Qt формируется sectionSizeFromContents. Может пригодится, удалять комментарий не нужно
    if (orientation() == Qt::Vertical) {
        ...
        auto variant = model()->headerData(logicalIndex, orientation(), Qt::DecorationRole);
        ...
        bool nullIcon = opt.icon.isNull();
        int margin = style()->pixelMetric(QStyle::PM_HeaderMargin, &opt, this);
        int iconSize = nullIcon ? 0 : style()->pixelMetric(QStyle::PM_SmallIconSize, &opt, this);
        QSize txt = opt.fontMetrics.size(0, opt.text);

        size.setHeight(margin + qMax(iconSize, txt.height()) + margin);
        sz.setWidth((nullIcon ? 0 : margin) + iconSize
                    + (opt.text.isNull() ? 0 : margin) + txt.width() + margin);
    ...
    for (int level = 0; level < depth; ++level) {
        ...
        if (cell_rect.width() > 0 && cell_rect.height() > 0) {
            ...
            if (orientation() == Qt::Horizontal && isSortIndicatorShown() && sortIndicatorSection() == section_from
                ...
                QRect sort_rect = cell_rect;

                const int sort_size = Utils::scaleUI(8);
                sort_rect.moveTo(sort_rect.left() + (sort_rect.width() - sort_size) / 2,
                                 sort_rect.top() - sort_size / 4 + Utils::scaleUI(1));
                sort_rect.setWidth(sort_size);
                sort_rect.setHeight(sort_size);

                QIcon icon;
                if (sortIndicatorOrder() == Qt::AscendingOrder)
                    icon = Utils::resizeSvg(":/share_icons/sort-up.svg", sort_size, true);
                else
                    icon = Utils::resizeSvg(":/share_icons/sort-down.svg", sort_size, true);
                icon.paint(painter, sort_rect);
...
DatasetConfigOptions HeaderView::configMenuOptions() const
{
    return _config_options;
}

void HeaderView::setConfigMenuOptions(const DatasetConfigOptions& o)
{
    _config_options = o;
}

void HeaderView::setAutoSearchEnabled(bool b)
{
    _show_autosearch = b;
}

bool HeaderView::isAutoSearchEnabled() const
{
    return _show_autosearch;
}

const View* HeaderView::view() const
{
    return _view;
}

DataProperty HeaderView::datasetProperty() const
{
...
void HeaderView::reloadDataFromRootItemHelper()
{
    ...
    for (auto h : qAsConst(all_bottom)) {
        ...
        if (!is_out_of_limit) {
            ...
            if ((h->resizeMode() == Interactive || h->resizeMode() == Fixed) && sectionSize(h->sectionFrom()) != h->sectionSize())
...
{
    // борьба с багом, при котором Qt выдает в QItemSelectionModel::select
    // qWarning("QItemSelectionModel: Selecting when no model has been set will result in a no-op.");
    if (((_view == nullptr && _dataset_property.isValid()) || (_view != nullptr && _view->objectExtensionDestroyed()))
