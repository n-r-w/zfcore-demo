...
public:
    ...
    bool mWidgetMovable = false;
    bool mWidgetResizable = false;
    bool mUseRubberBandOnResize = false;
    bool mUseRubberBandOnMove = false;
};
...
void NcCursorPosCalculator::recalculate(const QPoint& globalMousePos, const QRect& frameRect)
{
    ...
    onLeftEdge = globalMouseX >= frameX && globalMouseX <= frameX + mBorderWidth;

    onRightEdge = globalMouseX >= frameX + frameWidth - mBorderWidth && globalMouseX <= frameX + frameWidth;

    onTopEdge = globalMouseY >= frameY && globalMouseY <= frameY + mBorderWidth;

    onBottomEdge = globalMouseY >= frameY + frameHeight - mBorderWidth && globalMouseY <= frameY + frameHeight;
...
bool NcFramelessHelper::eventFilter(QObject* obj, QEvent* event)
{
    ...
    if (type == QEvent::MouseMove || type == QEvent::HoverMove || type == QEvent::MouseButtonPress
        || type == QEvent::MouseButtonRelease || type == QEvent::Leave) {
