...
{
    Z_CHECK(!rootHeaderItem(Qt::Horizontal)->isUpdating());
...
void TableView::setBooleanColumn(int logical_index, bool is_boolean)
{
    if (is_boolean) {
...
bool TableView::isBooleanColumn(int logical_index) const
{
...
int TableView::checkedRowsCount() const
{
    if (isAllRowsChecked() && _checked.isEmpty())
        return Utils::getAllRowCount(Utils::getTopSourceModel(model()));
...
void TableView::clearAllCheckRows()
{
    checkAllRows(false);
}
...
void TableView::setAutoShrink(bool b)
{
    if (b == _auto_shrink)
        return;
...
void TableView::customEvent(QEvent* event)
{
    ...
    if (event->type() == UPDATE_FROZEN_PROPERTIES_EVENT) {
        if (_frozen_table_view != nullptr) {
            ...
            _frozen_table_view->setConfigMenuOptions(configMenuOptions());
            ...
            _frozen_table_view->setShowGrid(showGrid());
        }
...
void TableView::setNoEitTriggersColumn(int logical_index, bool no_edit_triggers)
{
    if (no_edit_triggers) {
...
bool TableView::isNoEditTriggersColumn(int logical_index) const
{
    return _no_edit_triggers_columns.contains(logical_index);
}

void TableView::setModernCheckBox(bool b)
{
...
bool TableView::isModernCheckBox() const
{
...
void TableView::updateCornerWidget()
{
    ...
    if (_corner_text.isEmpty()) {
        ...
        _corner_label->setGeometry(_corner_margin_left, _corner_margin_top,
            (verticalHeader() != nullptr ? verticalHeader()->width() : 0) - _corner_margin_left - _corner_margin_right,
            (horizontalHeader() != nullptr ? horizontalHeader()->height() : 0) - _corner_margin_top - _corner_margin_bottom);
...
void TableView::sl_rowsRemoved(const QModelIndex& parent, int first, int last)
{
    ...
    if (!_checked.isEmpty()) {
        for (int row = first; row <= last; row++) {
            int sourceRow = Utils::getTopSourceIndex(model()->index(row, 0)).row();
            _checked.remove(sourceRow);
        }

        // Оперируем индексами модели, а не вьюшки, пересчет в данный момент не нужен
        // коррекция
...
void TableView::updateFrozenCount()
{
    if (_frozen_group_count == 0) {
        ...
        if (_frozen_table_view == nullptr) {
            ...
            if (view() != nullptr)
                _frozen_table_view = new FrozenTableView(this, view(), datasetProperty(), this);
            else
...
{
    _block_update_geometry_count--;
    Z_CHECK(_block_update_geometry_count >= 0);
...
TableView::TableView(const DataStructurePtr& structure, const DataProperty& dataset_property, QWidget* parent)
    : TableViewBase(structure, dataset_property, parent)
{
    init();
}

TableView::TableView(const View* view, const DataProperty& dataset_property, QWidget* parent)
    : TableViewBase(view, dataset_property, parent)
{
    init();
}

TableView::TableView(const DataWidgets* widgets, const DataProperty& dataset_property, const HighlightProcessor* highlight, QWidget* parent)
    : TableViewBase(widgets, dataset_property, highlight, parent)
{
    init();
}
    ...
    if (_frozen_table_view) {
        ...
        if (!_need_update_frozen_properties
            && (_frozen_table_view->isSortingEnabled() != isSortingEnabled() || _frozen_table_view->editTriggers() != editTriggers()
                ...
                || _frozen_table_view->configMenuOptions() != configMenuOptions() || _frozen_table_view->selectionMode() != selectionMode() || _frozen_table_view->toolTip() != toolTip()
                || _frozen_table_view->verticalHeader()->defaultSectionSize() != verticalHeader()->defaultSectionSize() || _frozen_table_view->showGrid() != showGrid()
