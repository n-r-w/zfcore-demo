...
HANDLE v_hPrivateHeap = nullptr;
CRITICAL_SECTION v_csecThreadSynch;
HICON v_icoOffDocIcon = nullptr;
ULONG v_cLocks = 0;
BOOL v_fUnicodeAPI = 0;
BOOL v_fWindows2KPlus = 0;
...
STDAPI_(LPWSTR) DsoConvertToLPWSTR(LPCSTR pszMbcsString)
{
    ...
    if ((pszMbcsString) && ((cblen = lstrlen(pszMbcsString)) > 0)) {
...
STDAPI_(LPSTR) DsoConvertToMBCS(LPCWSTR pwszUnicodeString)
{
    ...
    if ((pwszUnicodeString) && ((cblen = lstrlenW(pwszUnicodeString)) > 0)) {
        cbnew = ((cblen + 1) * sizeof(WCHAR));
...
STDAPI_(LPWSTR) DsoCopyStringCatEx(LPCWSTR pwszBaseString, UINT cStrs, LPCWSTR *ppwszStrs)
{
    ...
    if ((pwszBaseString == nullptr) || ((cblenb = lstrlenW(pwszBaseString)) < 1))
        return nullptr;
...
STDAPI_(UINT) DsoCompareStringsEx(LPCWSTR pwsz1, INT cch1, LPCWSTR pwsz2, INT cch2)
{
    ...
    if ((pwsz1 == nullptr) || (cch1 == 0) || ((cblen1 = ((cch1 > 0) ? cch1 : lstrlenW(pwsz1))) == 0))
        return CSTR_LESS_THAN;

    if ((pwsz2 == nullptr) || (cch2 == 0) || ((cblen2 = ((cch2 > 0) ? cch2 : lstrlenW(pwsz2))) == 0))
    ...
    if (cblen1 == cblen2) {
        for (iret = 0; iret < cblen1; iret++) {
            ...
            if (((pwsz1[iret] >= 'A') && (pwsz1[iret] <= 'Z')) && ((pwsz1[iret] + ('a' - 'A')) == pwsz2[iret]))
                continue;

            if (((pwsz2[iret] >= 'A') && (pwsz2[iret] <= 'Z')) && ((pwsz2[iret] + ('a' - 'A')) == pwsz1[iret]))
                continue;
...
STDAPI_(BOOL) GetTempPathForURLDownload(WCHAR* pwszURL, WCHAR** ppwszLocalFile)
{
    ...
    if (!(pwszURL) || ((dw = lstrlenW(pwszURL)) < 6) || !(LooksLikeHTTP(pwszURL)) || !(ppwszLocalFile))
        return FALSE;
    ...
    *ppwszLocalFile = nullptr;
    ...
    if (GetTempPath(MAX_PATH, szTmpPath)) {
        ...
        if (CreateDirectory(szTmpPath, nullptr) || GetLastError() == ERROR_ALREADY_EXISTS) {
            lstrcat(szTmpPath, "\\");
    ...
    if (pwszTPath) {
        if ((pwszTFile = DsoCopyString(pwszURL)) != 0) {
...
STDAPI URLDownloadFile(LPUNKNOWN punk, WCHAR* pwszURL, WCHAR* pwszLocalFile)
{
    ...
    if (pfnURLDownloadToFileW == nullptr) {
        if ((hUrlmon = LoadLibrary("URLMON.DLL")) != 0)
...
STDAPI_(BOOL) IsWindowChild(HWND hwndParent, HWND hwndChild)
{
    ...
    while ((hwnd = GetParent(hwnd)) != nullptr)
        if (hwnd == hwndParent)
            return TRUE;
...
STDAPI DsoDispatchInvoke(LPDISPATCH pdisp, LPOLESTR pwszname, DISPID dspid, WORD wflags, DWORD cargs, VARIANT* rgargs, VARIANT* pvtret)
{
    ...
    DISPPARAMS dspparm = {NULL, nullptr, 0, 0};
    ...
    if (SUCCEEDED(hr))
        hr = pdisp->Invoke(
            dspid, IID_NULL, LOCALE_USER_DEFAULT, (WORD)(DISPATCH_METHOD | wflags), &dspparm, pvtret, nullptr, nullptr);
...
STDAPI DsoReportError(HRESULT hr, LPWSTR pwszCustomMessage, EXCEPINFO* peiDispEx)
{
    ...
    // Set the source name...
    bstrSource = SysAllocString(L"ExperiumMSOFramerControl");
    ...
    } else if (((nID) && LoadString(hProgInstance, nID, szError, sizeof(szError)))
...
{
    return (((px->vt & (VT_BYREF | VT_ARRAY)) == (VT_BYREF | VT_ARRAY))
            ? *(px->pparray)
...
{
    return (((px->vt == VT_DISPATCH) || (px->vt == VT_UNKNOWN))
            ? px->punkVal
            : (((px->vt == (VT_DISPATCH | VT_BYREF)) || (px->vt == (VT_UNKNOWN | VT_BYREF))) ? *(px->ppunkVal)
                                                                                             : nullptr));
}
...
STDAPI_(BOOL) FIsIECacheFile(LPWSTR pwszFile)
{
    ...
    if (RegOpenKey(HKEY_CURRENT_USER, "Software\\Microsoft\\Windows\\CurrentVersion\\Explorer\\Shell Folders", &hk)
        == ERROR_SUCCESS) {
        DWORD dwT, dwS;
        dwT = MAX_PATH;
        ...
        if (v_fUnicodeAPI) {
            if ((RegQueryValueExA(hk, "Cache", 0, &dwT, rgbuffer, &dwS) == ERROR_SUCCESS) && (dwT == REG_SZ)
                && (dwS > 1))
                pwszTmpCache = DsoConvertToLPWSTR((LPSTR)rgbuffer);
        } else {
            if ((RegQueryValueExW(hk, L"Cache", 0, &dwT, rgbuffer, &dwS) == ERROR_SUCCESS) && (dwT == REG_SZ)
                && (dwS > 1))
        ...
        if (pwszTmpCache)
		{
            ...
            fIsCacheFile = ((dwS < dwT) && (DsoCompareStringsEx(pwszTmpCache, dwS, pwszFile, dwS) == CSTR_EQUAL));
...
STDAPI_(BOOL) FGetPrinterSettings(HANDLE hprinter, LPWSTR *ppwszProcessor, LPWSTR *ppwszDevice, LPWSTR *ppwszOutput, LPDEVMODEW *ppdvmode, DWORD *pcbSize)
{
    ...
    if ((ppwszProcessor == nullptr) || (ppwszDevice == nullptr) || (ppwszOutput == nullptr) || (ppdvmode == nullptr)
        || (pcbSize == nullptr))
        return FALSE;
						  ...
						  BSTR *pbstrFile, BOOL *pfReadOnly)
{
    ...
    *pbstrFile = nullptr;
    ...
    } else { // If not, then we use OPENFILENAMEA and thunk down our params to
...
void init_mso_utilities()
{
    if (v_hPrivateHeap != nullptr)
        return;
