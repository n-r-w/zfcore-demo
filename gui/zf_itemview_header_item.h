...
class ZCORESHARED_EXPORT HeaderItem : public QObject
{
    Q_OBJECT
public:
    ~HeaderItem() override;
    ...
    //! Найти узел по его идентификатору. Только для корневого узла. Ищет рекурсивно во всех дочерних узлах
    HeaderItem* item(const PropertyID& property_id,
                     //! Если не найден - критическая ошибка
                     bool halt_if_not_found = true) const;
    //! Найти узел по его идентификатору. Только для корневого узла. Ищет рекурсивно во всех дочерних узлах
    HeaderItem* item(const DataProperty& property,
                     //! Если не найден - критическая ошибка
                     bool halt_if_not_found = true) const;
    ...
    //! Добавить дочерний узел
    HeaderItem* append(
        //! Идентификатор свойства. Должен быть уникальным в рамках всего заголовка
        const PropertyID& id,
        //! Текст заголовка
        const QString& label = QString());
    //! Добавить дочерний узел
    HeaderItem* append(
        //! Свойство. Должно быть уникальным в рамках всего заголовка
        const DataProperty& property,
        //! Текст заголовка. Если не задан, то берется название свойства
        const QString& label = QString());
    ...
    HeaderItem* insert(
        //! Позиция вставки
        const DataProperty& property_pos,
        //! Идентификатор. Должен быть уникальным в рамках всего заголовка
        int id,
        //! Текст заголовка
        const QString& label = QString());
    ...
    void remove(const PropertyID& property_id);
    //! Удалить дочерний узел
    void remove(const DataProperty& property);

    //! Поместить items_pos в новый заголовок на место target_pos
    void createHierarchy(
        //! Куда вставить новый заголовок. Если -1, то в конец
        int target_id,
        //! Идентификатор нового заголовка
        int hierarchy_id,
        //! Текст нового заголовка
        const QString& hierarchy_label,
        //! Какие заголовки переместить в новый (идентификаторы)
        QPair<int, int> range_ids,
        //! Имена перемещенных заголовков. Количество равно нулю или разнице в range_ids
        const QStringList& items_name = {});
    //! Поместить items в новый заголовок на место target_prop
    void createHierarchy(
        //! Куда вставить новый заголовок. Если не задано, то в конец
        const PropertyID& target_prop,
        //! Идентификатор нового заголовка
        int hierarchy_id,
        //! Текст нового заголовка
        const QString& hierarchy_label,
        //! Какие заголовки переместить в новый
        QPair<PropertyID, PropertyID> range_props,
        //! Имена перемещенных заголовков
        const QStringList& items_name = {});
...
private:
    ...
    FeedbackTimer* _recalc_timer = nullptr;
