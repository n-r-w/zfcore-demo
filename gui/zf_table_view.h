...
class ZCORESHARED_EXPORT TableView : public TableViewBase
{
    Q_OBJECT
public:
    ...
    void setCornerText(
        const QString& text, Qt::Alignment alignment = Qt::AlignCenter, int margin_left = 4, int margin_right = 4, int margin_top = 4, int margin_bottom = 4);
    ...
    //! Отметить указанную колонку как boolean. При этом двойной клик или нажатие на пробел будет переключать состояние true/false
    void setBooleanColumn(int logical_index, bool is_boolean);
    //! Является ли колонка логической
    bool isBooleanColumn(int logical_index) const override;
    ...
    bool isNoEditTriggersColumn(int logical_index) const override;

    //! Показывать чекборкс как круглую иконку
    void setModernCheckBox(bool b);
    //! Показывать чекборкс как круглую иконку
    bool isModernCheckBox() const override;
