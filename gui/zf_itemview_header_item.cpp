...
HeaderItem* HeaderItem::item(int id, bool halt_if_not_found) const
{
    ...
    if (halt_if_not_found)
        Z_CHECK_X(res != nullptr, "HeaderItem::item, item not found");

    return res;
}

HeaderItem* HeaderItem::item(const PropertyID& property_id, bool halt_if_not_found) const
{
    return item(property_id.value(), halt_if_not_found);
}

HeaderItem* HeaderItem::item(const DataProperty& property, bool halt_if_not_found) const
{
    return item(property.id(), halt_if_not_found);
}
...
int HeaderItem::visualToLogical(int pos) const
{
    ...
    Z_CHECK(pos < visual_items.count());
...
{
    return setDefaultSectionSize(fontMetrics().horizontalAdvance(Consts::AVERAGE_CHAR) * c + margin() * 2);
}
...
HeaderItem* HeaderItem::setSectionSizeCharCount(int c)
{
    ...
    return setSectionSize(fontMetrics().horizontalAdvance(Consts::AVERAGE_CHAR) * c + margin() * 2);
}
...
{
    Z_CHECK(start_section <= end_section);
    int size = 0;
...
{
    Z_CHECK(orientation() == Qt::Vertical);
    return levelSize();
}
...
{
    return setVerticalCellWidth(fontMetrics().horizontalAdvance(Consts::AVERAGE_CHAR) * c + margin() * 2);
}
...
QByteArray HeaderItem::toByteArray() const
{
    QByteArray data;
...
void HeaderItem::fromByteArrayHelper(QDataStream& ds)
{
    ...
    Z_CHECK(ds.status() == QDataStream::Ok);
}

void HeaderItem::fromByteArray(const QByteArray& data)
{
    beginUpdate();
    ...
    int version;
    ds >> version;
    Z_CHECK(version == _header_item_structure_version);

    bool is_root;
    ds >> is_root;
    Z_CHECK(is_root == isRoot());
...
HeaderItem* HeaderItem::setPermanentHidden(bool b)
{
    if (isRoot()) {
        for (int i = 0; i < bottomCount(); i++) {
            if (i == 0 && !b) {
                // иначе вообще ничего не будет видно
                bottomItem(i)->setPermanentHidden(false);
                bottomItem(i)->setVisible(true);

            } else {
                bottomItem(i)->setPermanentHidden(b);
            }
        }
...
HeaderItem* HeaderItem::topParent() const
{
    ...
    Z_CHECK(false); // тут быть не должны
    return nullptr;
}
...
{
    Z_CHECK(visual_pos >= 0 && visual_pos < count());
...
{
    if (isRoot()) {
        Z_CHECK(section >= 0 && section < root()->sectionSpan());
...
int HeaderItem::bottomCount(bool visible_only) const
{
    ...
    if (isRoot() && _update_counter == 0)
        Z_CHECK(qMax(0, root()->sectionSpan()) == res);
#endif
...
HeaderItem* HeaderItem::append(const PropertyID& id, const QString& label)
{
    Z_CHECK(id.isValid());
    return append(id.value(), label);
}

HeaderItem* HeaderItem::append(const DataProperty& property, const QString& label)
{
    Z_CHECK(property.isValid() && property.isColumn());

    QString translation;
    if (label.isEmpty()) {
        if (property.options().testFlag(PropertyOption::NoTranslate))
            translation = property.translationID();
        else
            translation = translate(property.translationID());
    } else {
        translation = label;
    }

    return append(property.id().value(), translation);
}

HeaderItem* HeaderItem::insert(int pos, int id, const QString& label)
{
    Z_CHECK(pos >= 0 && pos <= count() && id >= 0);
    auto check_item = item(id, false);
    Z_CHECK(check_item == nullptr);
...
HeaderItem* HeaderItem::insert(int pos, const DataProperty& property, const QString& label)
{
    Z_CHECK(property.isValid() && property.isColumn());
    ...
    return insert(pos, property.id().value(), translation);
}

HeaderItem* HeaderItem::insert(int pos, const PropertyID& property_id, const QString& label)
{
    return insert(pos, item(property_id)->pos(), label);
}

HeaderItem* HeaderItem::insert(const DataProperty& property_pos, int id, const QString& label)
{
    return insert(item(property_pos)->pos(), id, label);
}

HeaderItem* HeaderItem::insert(const PropertyID& property_pos, int id, const QString& label)
{
    return insert(item(property_pos)->pos(), id, label);
}

HeaderItem* HeaderItem::insert(const PropertyID& property_pos, const DataProperty& property, const QString& label)
{
    return insert(item(property_pos)->pos(), item(property)->pos(), label);
}

HeaderItem* HeaderItem::insert(const PropertyID& property_pos, const PropertyID& property_id, const QString& label)
{
    return insert(item(property_pos)->pos(), item(property_id)->pos(), label);
}

HeaderItem* HeaderItem::insert(const DataProperty& property_pos, const PropertyID& property_id, const QString& label)
{
    return insert(item(property_pos)->pos(), item(property_id)->pos(), label);
}
...
void HeaderItem::remove(const PropertyID& property_id)
{
    remove(item(property_id)->pos());
}

void HeaderItem::remove(const DataProperty& property)
{
    remove(property.id());
}

void HeaderItem::createHierarchy(int target_id, int hierarchy_id, const QString& hierarchy_label, QPair<int, int> range_ids,
                                 const QStringList& items_name)
{
    Z_CHECK(target_id < 0 || contains(target_id));
    Z_CHECK(!hierarchy_label.isEmpty());

    Z_CHECK(contains(range_ids.first));
    Z_CHECK(contains(range_ids.second));
    ...
    int pos_1 = qMin(item(range_ids.first)->pos(), item(range_ids.second)->pos());
    int pos_2 = qMax(item(range_ids.first)->pos(), item(range_ids.second)->pos());

    Z_CHECK(items_name.isEmpty() || items_name.count() == pos_2 - pos_1 + 1);

    for (int i = pos_2; i >= pos_1; i--) {
        QString name;
        if (!items_name.isEmpty())
            name = items_name.at(i - pos_1);
        else
            name = child(pos_1)->label();

        new_names.prepend(name);
        new_ids.prepend(child(i)->id());

        remove(i);
    }

    auto h = insert(target_id >= 0 ? item(target_id)->pos() : count(), hierarchy_id, hierarchy_label);
    for (int i = 0; i < new_ids.count(); i++) {
        h->append(new_ids.at(i), new_names.at(i));
    }

    endUpdate();
}

void HeaderItem::createHierarchy(const PropertyID& target_prop, int hierarchy_id, const QString& hierarchy_label,
                                 QPair<PropertyID, PropertyID> range_props, const QStringList& items_name)
{
    createHierarchy(target_prop.isValid() ? item(target_prop)->id() : -1, hierarchy_id, hierarchy_label,
                    QPair<int, int> {item(range_props.first)->id(), item(range_props.second)->id()}, items_name);
}
    ...
    , _orientation(orientation)
{
    ...
    if (orientation == Qt::Horizontal) {
        ...
        setDefaultSectionSize(UiSizes::defaultTableRowHeight());
    }

    if (isRoot()) {
        _recalc_timer = new FeedbackTimer(this);
        connect(_recalc_timer, &FeedbackTimer::timeout, this, [&]() { calculateSectionsSizeHelper(); });
    } else {
        Z_CHECK(isEmpty());
    }
...
void HeaderItem::sortHelper(HeaderItem* item, Qt::SortOrder order)
{
    ...
    Z_CHECK(item == nullptr || item->isBottom());
    Z_CHECK(orientation() == Qt::Horizontal);
...
{
    Z_CHECK(logical_section >= 0 && logical_section < root()->sectionSpan());
...
{
    Z_CHECK(visual_section >= 0 && visual_section < root()->sectionSpan());
...
void HeaderItem::setSectionsSizes(const QMap<int, int>& sizes)
{
    ...
    QSet<int> not_used(Utils::toSet(s_keys));
    for (auto h : items) {
...
{
    Z_CHECK(_orientation == Qt::Vertical);
...
int HeaderItem::calculateSpan(int level, int section)
{
    ...
    if (isRoot()) {
        ...
        _section_span = section_count;
        _level_span_to = _level_from + _level_span - 1;
...
HeaderItem *HeaderItem::findByPosHelper(int level, int section, bool use_span) const
{
    if (!isRoot()) {
        if (use_span) {
            if (level >= _level_from && (level <= _level_span_to || isBottom()) && section >= _section_from && section <= _section_span_to)
                return const_cast<HeaderItem*>(this);
        } else {
